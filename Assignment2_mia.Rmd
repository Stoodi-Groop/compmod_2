...
m1 <- brm(
  ...
  prior = m0_p2,
  sample_prior= T, 
  #file = "m1" comenting out this because i want to overwrite 
)
...
hypothesis(m1, "Diagnosis > 0.1") #given our model, what are the chances that the effect of Diagnosis is above 0
```
...
```{r}
#lolling around with priors
#defining priors
m_vh_mr_p2 <- c(
  prior(normal(0, 1), class = Intercept), #since we scaled the data, these priors for intercept are reasonable
  prior(normal(1, 0.5), class = sigma), #we expect the model to perform as bad as the mean
  prior(normal(0, 0.5), class = b, coef = VH ),
  prior(normal(0, 0.5), class = b, coef = MR)
)
...
#posterior stuff
patients_posterior2 <-  posterior_samples(m2_2)

#plotting the beta values
ggplot(patients_posterior2) +
  theme_classic() +
  geom_density(aes(prior_b_MR), fill="red", alpha=0.3) +
  geom_density(aes(b_MR), fill="blue", alpha=0.5)
